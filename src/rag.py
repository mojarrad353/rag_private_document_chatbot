...
class RAGService:
    ...
    def process_file(self, session_id: str, filepath: str) -> None:
        ...
        # Since Chroma in-memory is tricky with separation, we'll try to use a simple instance per user
        # or just re-create it. content of 'texts' is what matters.
        
        docsearch = Chroma.from_documents(
            texts, 
            embeddings,
            # collection_name=f"session_{session_id}" # Optional: if using persistent client
        )
